...
namespace NesEmulator.Processor
{
    ...
    {
        public class CompareStrategy : AutoIncrementInstructionPointerStrategyBase
        {
            protected override void ExecuteImpl(CPU cpu, OpCode opcode, byte firstOperand, IMemoryBus memoryBus)
            {
                byte comparisonValue = GetComparisonValue(cpu, opcode, firstOperand, memoryBus);
            ...
            private byte GetComparisonValue(CPU cpu, OpCode opcode, byte operand, IMemoryBus memoryBus)
            {
                int cyclePenalty = 0;
